  ...
  const notifyZone = async([zoneKey, text]) => {
    return Promise.all(users.map(async(user) => {
      ...
      return contacts && Promise.all(contacts.map((contact) => {
        return sendPush(user.subscription, `Your goal date for contacting ${contact.firstName} ${contact.lastName} ${text}`);
      }));
    }));
  };

  const zones = [['yellowZoneStartDate', 'is approaching'], 
    ['redZoneStartDate', 'is coming very soon!'],
    ['deadlineDate', 'has passed.']];
